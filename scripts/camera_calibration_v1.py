# -*- coding: utf-8 -*-
"""
Created on Tue Dec 12 10:28:59 2023
...
# calibration
def calibrate_camera(images_folder, camera_index, calibration_frame_range=(120, 300), save_folder='plots', max_frames=130):
    start_frame, end_frame = calibration_frame_range
    calibration_frames = [f'frame_{i:04d}.png' for i in range(start_frame, end_frame + 1)]
    ...
    for imname in calibration_frames:
        im_path = os.path.join(images_folder, imname)
        if os.path.exists(im_path):
            ...
            if not ret:
                ...
                print(f"Could not find corners in image. Skipping: {imname}")
                continue
            ...
            new_filename = f'calibration_frame_cam{camera_index}_{int(imname.split("_")[1].split(".")[0]):04d}.png'
...
def stereo_calibrate(mtx1, dist1, mtx2, dist2, num_pairs=50, save_folder='plots'):
    ...
    save_selected_pairs_to_csv(selected_frame_numbers, csv_filename=os.path.join(save_folder, 'selected_calibration_pairs.csv'))
    ...
    save_metrics_to_csv(metrics, csv_filename=os.path.join(
        save_folder, 'stereo_calibration_metrics.csv'))
    ...
    if save_folder:
        ...
        save_plot(plt, "stereo_calibration_plot.png", save_folder)
    else:
        plt.show()
...
# Plot the original and reprojected points
    ...
    if save_folder:
        save_plot(plt, "reprojection_plot.png", save_folder)
    else:
        plt.show()
...
# change range to what is applicable
    save_folder = r"C:\Users\valer\OneDrive - Wageningen University & Research\experiment_Alg4_221_pvc\calibration"
    mtx1, dist1 = calibrate_camera(images_folder=r"C:\Users\valer\OneDrive - Wageningen University & Research\experiment_Alg4_221_pvc\frames_left",
                               camera_index=1, calibration_frame_range=(80, 1000), save_folder=save_folder)
    mtx2, dist2 = calibrate_camera(images_folder=r"C:\Users\valer\OneDrive - Wageningen University & Research\experiment_Alg4_221_pvc\frames_right",
                               camera_index=2, calibration_frame_range=(80, 1000), save_folder=save_folder)
